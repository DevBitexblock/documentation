...
### What’s the Bitexblock Prediction contract address?
...
The treasury fee is currently set at 3%: this may be subject to changes, which would be announced on Bitexblock’s official communication channels. Treasury fees are used to buy back and burn BIT tokens.

### **What are the fees?**

3% of each round's total pot will go to the treasury, which will be used to buyback and burn BIT every Monday.

### What are you using for your price feed?

Bitexblock uses two sources for our price feeds. They each have their own purpose within the prediction market:
...
* Used for real-time price updates on the Bitexblock prediction market interface.
...
In the very rare occurrence that the Locked Price is exactly the same as the Closed Price, no one wins, and all funds entered into positions will be sent to the treasury to be used for BIT buybacks to burn.
...
### What causes Bitexblock Prediction market to pause?
